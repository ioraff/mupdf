...
ifeq ($(HAVE_WAYLAND),yes)
  ...
  MUVIEW_WAYLAND_OBJ += $(OUT)/platform/x11/pdfapp.o
  MUVIEW_WAYLAND_OBJ += $(OUT)/platform/x11/wl_main.o
  $(MUVIEW_WAYLAND_EXE) : $(MUVIEW_WAYLAND_OBJ) $(MUPDF_LIB) $(THIRD_LIB) $(PKCS7_LIB)
	$(LINK_CMD) $(THIRD_LIBS) $(WAYLAND_LIBS) $(LIBCRYPTO_LIBS)
  INSTALL_APPS += $(MUVIEW_WAYLAND_EXE)
endif
...
ifeq ($(HAVE_WAYLAND),yes)
ifeq ($(HAVE_CURL),yes)
  MUVIEW_WAYLAND_CURL_EXE := $(OUT)/mupdf-x11-curl
  MUVIEW_WAYLAND_CURL_OBJ += $(OUT)/platform/x11/curl/pdfapp.o
  MUVIEW_WAYLAND_CURL_OBJ += $(OUT)/platform/x11/curl/wl_main.o
  MUVIEW_WAYLAND_CURL_OBJ += $(OUT)/platform/x11/curl/curl_stream.o
  $(MUVIEW_WAYLAND_CURL_EXE) : $(MUVIEW_WAYLAND_CURL_OBJ) $(MUPDF_LIB) $(THIRD_LIB) $(PKCS7_LIB) $(CURL_LIB)
	$(LINK_CMD) $(THIRD_LIBS) $(WAYLAND_LIBS) $(CURL_LIBS) $(LIBCRYPTO_LIBS)
  EXTRA_APPS += $(MUVIEW_WAYLAND_CURL_EXE)
endif
endif
